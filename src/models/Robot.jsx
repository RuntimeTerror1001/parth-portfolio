...
import { useGLTF, useAnimations, useDepthBuffer } from '@react-three/drei'
import {useFrame, useThree} from '@react-three/fiber'
import {a} from '@react-spring/three'

import robotScene from '../assets/3d/robot_playground.glb'

const Robot = ({color = '#6b2737',props}) => {
    ...
    useEffect(() => {
      ...
      // Ensure we're not looping
      action.loop = useThree.LoopOnce;
      action.clampWhenFinished = true;
      action.repetitions = 1;
      
      // Function to handle the first complete playthrough
      const handleFirstPlaythrough = () => {
        ...
        // Check time on each frame
        const pauseAtTime = () => {
          if (action.time >= 4) {
            // Reached our target time, pause it
            action.paused = true;
          } else if (!action.paused) {
            // Continue checking on next frame if still playing
            animationFrameId = requestAnimationFrame(pauseAtTime);
          }
        };
      ...
      // Set up a listener that fires only once
      const onFinished = () => {
        if (!firstPlayCompleted) {
          action.getMixer().removeEventListener('finished', onFinished);
          handleFirstPlaythrough();
        }
      };
      ...
      // Clean everything up when component unmounts
      return () => {
        action.getMixer().removeEventListener('finished', onFinished);
        if (animationFrameId) {
